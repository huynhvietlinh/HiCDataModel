...
// Parse the parameter
void parse_parameter(int argc, char** argv, string& hic_filename, string& hic_file_format, ModelFittingParameter& model_fitting_parameter) {
  if (argc % 2 == 0) {
  ...
  for (int i = 1; i < argc; i = i + 2) {
    ...
    if (option.compare("-fn") == STR_EQ)
      hic_filename = argv[i+1];
    else if (option.compare("-ff") == STR_EQ)
      hic_file_format = argv[i+1];
...
int main (int argc, char** argv) {
  string hic_filename, hic_file_format, output_filename;
  ModelFittingParameter model_fitting_parameter;
  parse_parameter(argc, argv, hic_filename, hic_file_format, model_fitting_parameter);
  // Read Hi-C data
  ValueMatrix hi_c_mat;
  if (hic_file_format.compare("full_matrix_format") == STR_EQ)
    read_full_hic_data_file (hic_filename, 0, 0, hi_c_mat);
  else if (hic_file_format.compare("sparse_matrix_format") == STR_EQ)
    read_sparse_hic_data_file (hic_filename, model_fitting_parameter.resolution, hi_c_mat);
  else {
    cout << "Can not find the format " << hic_file_format << endl;
    return -1;
  }
...
/*int fit_model(int argc, char** argv) {
  if (argc < 10) {
    cout << "Need 10 parameters!" << endl;
    return -1;
  }

  string hi_c_filename = argv[1];
  string hi_c_file_format = argv[2];
  int resolution = atoi(argv[3]);
  int left = atoi(argv[4]);
  int right = atoi(argv[5]);
  int min_length_obj = atoi(argv[6]);
  int mid_length_obj = atoi(argv[7]);
  int max_length_obj = atoi(argv[8]);
  string fitting_method = argv[9];
  int segmentation_length = ((argc >= 12)? atoi(argv[10]):0);
  int min_overlap_length = ((argc >= 12)? atoi(argv[11]):0); 
  double zero_weight = ((argc >= 13)? atof(argv[12]):0);

  ValueMatrix hi_c_mat;
  if (hi_c_file_format.compare("full_matrix_format") == STR_EQ)
    read_hi_c_data (hi_c_filename, 0, 0, hi_c_mat);
  else if (hi_c_file_format.compare("sparse_matrix_format") == STR_EQ)
    read_hi_c_data (hi_c_filename, resolution, hi_c_mat);
  else {
    cout << "Can not find the format " << hi_c_file_format << endl;
    return -1;
  }
  
  if (right <= 0)
    right = hi_c_mat.size() - 1;
  
  //cout << hi_c_matrix.size() << endl;
  //for (int i = 0; i < hi_c_matrix.size(); i++)
  //	cout << hi_c_matrix[i][i] << endl;

  cout << "(left,right,min,mid,max) = " << left << "," << right << "," 
       << min_length_obj << "," << mid_length_obj << "," << max_length_obj << endl;  
  
  HiCDataModel hi_c_data_model;
  hi_c_data_model.resolution = resolution;
  if (fitting_method.compare("full") == STR_EQ)
    fit_hi_c_data_model (hi_c_mat, left, right, min_length_obj, mid_length_obj, max_length_obj, zero_weight, hi_c_data_model, true);
  else if (fitting_method.compare("segmentation") == STR_EQ)
    approx_fit_hi_c_data_model (hi_c_mat, left, right, min_length_obj, mid_length_obj, max_length_obj, 
                                segmentation_length, min_overlap_length, zero_weight, hi_c_data_model, false);
  else {
    cout << "Can not find the fitting method " << fitting_method << endl;
  }

  //print_prediction(hi_c_mat, hi_c_data_model, zero_weight, "chr_1_test");

  string filename = hi_c_filename;
  std::replace(filename.begin(), filename.end(), '/', '_');
  filename = "Model/" + filename + "_" + num_to_string(left) + "_" + num_to_string(right) + "_" + num_to_string(min_length_obj) 
                + "_" + num_to_string(mid_length_obj) + "_" + num_to_string(max_length_obj);
  hi_c_data_model.write_to_file(filename, "Chromosome 22");
